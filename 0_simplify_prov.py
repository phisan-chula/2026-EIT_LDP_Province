#!/usr/bin/env python3
"""
Refactored Object-Oriented Province Simplification Pipeline with Directional Divider Line Splitting
"""
...
class ProvinceSimplifier:
    ...
    def apply_divider_splits(self):
        """5) Apply divider lines to split specified province polygons into _N (left) and _S (right) parts using vector cross-product math."""
        if self.divider_gdf is None or self.divider_gdf.empty:
            return
        ...
        for _, div_row in self.divider_gdf.iterrows():
            hasc_target = div_row["HASC_1"]
            cutter = div_row.geometry

            if cutter is None or cutter.is_empty:
                continue

            # Find matching province row in self.gdf
            ...
            # Perform split using shapely.ops.split
            split_result = split(poly_geom, cutter)
            parts = list(split_result.geoms)

            if len(parts) < 2:
                print(f"-> [WARNING] Divider for {hasc_target} did not successfully split the polygon.")
                continue

            rows_to_drop.append(match_idx[0])

            # Extract start (A) and end (B) points from the linestring cutter
            coords = list(cutter.coords)
            x1, y1 = coords[0]       # Start point A
            x2, y2 = coords[-1]      # End point B

            # Evaluate each part using its representative point P(x3, y3)
            # Position > 0 -> Left side (_N)
            # Position < 0 -> Right side (_S)
            for part in parts:
                p_pt = part.representative_point()
                x3, y3 = p_pt.x, p_pt.y

                position = (x2 - x1) * (y3 - y1) - (y2 - y1) * (x3 - x1)
                
                new_row = prov_row.copy()
                new_row.geometry = part
                
                # Calculate area using area_sqkm = area * 111 * 111
        ...
        if rows_to_drop:
            self.gdf = self.gdf.drop(index=rows_to_drop).copy()
