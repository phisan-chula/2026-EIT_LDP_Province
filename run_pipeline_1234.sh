...
# Run the full map-projection pipeline for every Thai province.
# Each province is processed sequentially through steps 1-4, while multiple
# provinces are processed concurrently.
...
export PYTHON_BIN SCRIPT_1 SCRIPT_2 SCRIPT_3 SCRIPT_4 OUTPUT_ROOT
...
check_requirements() {
    ...
    for script in "$SCRIPT_1" "$SCRIPT_2" "$SCRIPT_3" "$SCRIPT_4"; do
        if [[ ! -f "$script" ]]; then
...
run_python_step() {
    ...
    case "$step" in
        ...
        3)
            ...
            "$PYTHON_BIN" "$SCRIPT_3" -p "$province" --overwrite
            ;;
        4)
            # Script 4 expects the HASC_1 code as a positional argument:
            #     python3 4_analyse_ldp.py TH.CM
